...
private:
     ...
     */
    std::map<std::string,Employee*> Employee_Data;  //map to store the employee's data

    //ultility function recursivly add the subordinate of the provided id with the same provided department to the vector
    void recursive_department(const std::string& id,const std::string& department,std::vector<std::string> &department_partner) const;
