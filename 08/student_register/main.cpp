...
int main() {
    ...
    while(true) {
        ...
        if(command == "U" or command == "u") {
            ...
            std::vector<std::string> phone_num_vec = split(line, ' ', true);
            std::string phone_num,new_phone_num;
            ...
            if(student_numbers.find(id)==student_numbers.end()) {
                ...
                for(auto pair: student_numbers) {
                    if(pair.second->student_number==id){
                        old_phone=pair.second->phone_number;
                        std::cout<< old_phone <<std::endl;
                        std::cout<< "Enter a new phone number: ";
                        getline(std::cin,phone_num);
                        std::cout<<std::endl;
                        new_phone_num= phone_num.substr(0,3) + phone_num.substr(3,phone_num.length()-1);


                        if(is_valid_phone_number(phone_num)){
                            pair.second->phone_number=new_phone_num;
                        }
